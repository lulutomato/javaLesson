...
-- 필요시 참조관계일때 테이블 데이터 삭제 : 참조되는 데이터가 성적테이블에 있으면 학생테이블 데이터를 삭제 못 합니다.
...
INSERT INTO tbl_score(stuno,subject,jumsu,teacher,term)
VALUES (null,'국어',89,'이나연','2022_1');   --기본키를 구성하는 칼럼은 NULL 허용 못합니다
...
SELECT * FROM TBL_SCORE ts WHERE stuno = '2019019';				--검색 결과 n개
...
- 성적테이블에 '과목명'을 학생 1명이 동일한 과목을 수강한다면(예를 들면 1학년 국어 ,  2학년 국어, 3학년 국어 )
  ...
  2) '과목' 테이블 - 과목 코드, 과목명(중복허용),담당교사(중복허용)
    테이블은 최소로 데이터가 중복되도록 구성합니다. 데이터 중복은 데이터의 불일치를 발생시킵니다.
    테이블을 구성하는 과정을 '정규화' 잘 되어있다. => 테이블 구성을 잘했다.
