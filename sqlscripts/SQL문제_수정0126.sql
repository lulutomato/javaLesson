...
SELECT  tb.customID
FROM 
(SELECT SUM(QUANTITY *tp.PRICE)"총 구매금액"
FROM TBL_BUY tb 
JOIN TBL_PRODUCT tp 
ON tb.PCODE  = tp.PCODE)dd
WHERE dd BETWEEN 100000 AND 200000;
...
SELECT ac.QUANTITY, MAX(AGE)
from
(SELECT tc.AGE,tb.QUANTITY 
FROM TBL_CUSTOM tc, TBL_BUY tb 
WHERE  tc.CUSTOM_ID  = tb.CUSTOMID)ac
GROUP BY ac.QUANTITY,ac.AGE;
--B-3. 2023년 하반기 구매금액을 고객I-D별로 조회하시오. 금액이 높은 순서부터 조회하세요. (노희영)
SELECT tb.CUSTOMID,tp.PRICE  
FROM TBL_PRODUCT tp
JOIN TBL_BUY tb 
ON tp.PCODE = tb.PCODE
WHERE MONTHS_BETWEEN((TO_DATE('20230630'),TO_DATE('20231231'))
